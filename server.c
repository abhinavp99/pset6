...
char* indexes(const char* path)
{
    ...
    //char* php = NULL;
    //char* html = NULL;
    char* ht_copy = malloc(sizeof(p_copy) + 80);
    char* php_copy = malloc(sizeof(p_copy) + 80);
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    {   
        ...
        if(temp != temp1)
        {
            for(int i = 0; i < (long) temp2 - (long) temp; i++)
            {
                query[i] = temp2[i];
            }
        }
